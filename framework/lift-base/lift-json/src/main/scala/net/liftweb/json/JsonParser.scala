...
object JsonParser {
   ...
   * @throws ParseException is thrown if parsing fails
   */
  def parse(s: Reader): JValue = parse(s, true)

  /** Return parsed JSON.
   * @param closeAutomatically true (default) if the Reader is automatically closed on EOF
   * @throws ParseException is thrown if parsing fails
   */
  def parse(s: Reader, closeAutomatically: Boolean): JValue = 
    parse(new Buffer(s, true))
  ...
  /** Return parsed JSON.
   */
  def parseOpt(s: Reader): Option[JValue] = parseOpt(s, true)

  /** Return parsed JSON.
   * @param closeAutomatically true (default) if the Reader is automatically closed on EOF
   */
  def parseOpt(s: Reader, closeAutomatically: Boolean): Option[JValue] = 
